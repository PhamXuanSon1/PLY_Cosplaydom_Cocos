...
/**
 * Vì sao không có file nào để convert, kèm gợi ý sửa. Trường hợp hay gặp nhất là
 * chọn đúng định dạng mà file đang có (mp3 -> mp3) trong khi "Re-encode same
 * format" đang tắt.
 */
function describeNothingToDo(plan, options) {
    const rows = plan.rows || [];
    const sameFormat = rows.filter((r) => r.ok && r.skipped && r.sameFile);
    const exists = rows.filter((r) => r.ok && r.skipped && r.overwrites);
    const failed = rows.filter((r) => !r.ok);

    const lines = ['Không có file nào để convert.'];

    if (sameFormat.length) {
        lines.push(
            sameFormat.length + ' file đã đúng định dạng ' + plan.format.toUpperCase() +
            ' và ghi đè lên chính nó. Muốn hạ bitrate/sample rate thì bật "Re-encode same format", ' +
            'còn muốn đổi sang định dạng khác thì chọn lại ô Format.'
        );
    }
    if (exists.length) {
        lines.push(exists.length + ' file có sẵn ở đích rồi. Bật "Overwrite existing" nếu muốn ghi đè.');
    }
    if (failed.length) {
        lines.push(failed.length + ' file lỗi: ' + failed.slice(0, 3).map((r) => r.name + ' (' + r.error + ')').join(', '));
    }
    if (lines.length === 1) {
        lines.push('Danh sách file đang trống hoặc mọi file đều bị bỏ qua.');
    }
    return lines.join(' ');
}
...
exports.methods = {
    ...
    async run(options) {
        ...
        try {
            ...
            if (!todo.length) {
                // Trả về nguyên plan để panel liệt kê từng file bị bỏ qua vì lý do gì.
                // Báo "nothing to do" trống không thì không ai đoán được phải sửa gì.
                for (const r of plan.rows) {
                    log((r.ok ? 'BỎ QUA ' : 'LỖI ') + r.name + ': ' + (r.ok ? (r.reason || 'skipped') : r.error));
                }
                state.message = 'Không có gì để convert.';
                return {
                    ok: false,
                    error: describeNothingToDo(plan, options),
                    plan,
                    log: state.log.slice(),
                };
            }
